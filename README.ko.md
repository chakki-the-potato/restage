...
카드는 그 워크스페이스에 무엇이 들었는지 보여준다. 앞에 실제 앱 아이콘을 세 개까지 겹쳐
놓고 넘으면 `+2`로 센다. 그 아래에 배치 모양을 그림과 이름으로 적는다. 화면이 여러
대일 때만 몇 대인지 덧붙인다.

- 마우스를 올리면 단축키 칩이 사라지고 그 자리에 **편집**과 **더보기**가 들어온다
- 더보기에서 **이름 변경**, 단축키 변경, 파일로 열기, 삭제
- 단축키는 원하는 조합을 그냥 누르면 된다. config의 `hotkey` 줄만 바뀌고 나머지는 그대로다
- 삭제는 확인을 받고 휴지통으로 보낸다
- 실행하는 동안 어느 앱을 몇 개째 여는 중인지 카드에 보인다
- 실패하면 카드 안에 사유와 **다시 시도**가 붙는다
- config에 적힌 앱이 설치되어 있지 않으면 그 자리에 점선 표시와 함께 사유가 붙는다

머리말 오른쪽 **톱니**에서 로그인 시 자동 실행, **화면 모드**, **업데이트 확인**,
config 폴더 열기, 종료를 고른다.

다른 앱이 이미 쓰는 단축키는 등록되지 않는다. 그때도 카드에 사유가 뜬다.

### 화면 모드

기본은 시스템 밝기를 따른다. 톱니의 **화면 모드**에서 **시스템 설정 · 라이트 · 다크** 중
고를 수 있다. 고르면 바로 바뀐다. 패널을 띄워 둔 채 바꿔도 그 자리에서 바뀐다.
...
화면과 터미널 모두 영어와 한국어를 지원한다. 기본은 시스템 언어를 따르고, 패널 아래
**한국어 · English**에서 직접 고를 수 있다. 고르지 않았으면 시스템이 고른 쪽이 켜져
있다. 고르면 즉시 바뀐다. 다시 시작할 필요가 없다.
