...
**선언하지 않은 것은 손대지 않는다.** 파일에 없는 앱은 숨기지도 종료하지도 않고, 이미 열린
탭은 닫지 않고 없는 것만 추가한다. 유일한 예외가 `hideOthers`이고, 직접 켜야 동작한다.
...
Enter를 누르면 `~/.config/restage/dev.yaml`이 만들어진다. 그다음부터는 `restage open dev`로
다시 불러온다. 메뉴바의 **현재 창 배치로 새로 만들기**도 같은 일을 체크박스와 드롭다운으로
한다.

<img src="docs/images/editor-light.png" width="460" alt="창 목록을 보여주는 편집기. 같은 앱은 번호가 붙고 주소 없는 브라우저는 표시된다">

## 메뉴바

카드를 누르면 실행된다. 카드는 그 안에 무엇이 들었는지 보여준다. 실제 앱 아이콘을 세 개까지
겹쳐 놓고 넘으면 `+2`로 세며, 이름 아래에 배치 모양을 그린다. 마우스를 올리면 편집과 더보기가
나오고, 원하는 키 조합을 누르면 그 카드의 단축키가 된다.

톱니에는 로그인 시 자동 실행, 화면 모드, 순환 단축키, 업데이트 확인, config 폴더 열기,
종료가 있다.
...
hideOthers: false         # 선택. 실행할 때 여기 없는 앱을 숨긴다
...
정렬한 뒤 N번째(1부터). `any`는 현재 `builtin`과 동작이 같다.

**whenMissing** — 그 디스플레이가 연결되어 있지 않을 때 무엇을 할지. 기본값 `skip`은 그 화면을
버리고 사유를 보고한다. 거기 적힌 앱은 아예 열리지 않는다. `fullscreen`은 주 디스플레이로
옮기되 항목을 전부 전체화면으로 연다. 그러면 macOS가 데스크탑을 하나씩 내주므로 이미 있는
배치 위에 겹치지 않는다. 워크스페이스를 저장할 때 외장 화면에는 `fullscreen`이 자동으로
적힌다. 나갈 때보다 모니터가 적은 상황이 흔하기 때문이다.
...
**hideOthers** — 켜지 않으면 꺼져 있다. 켜면 워크스페이스를 실행한 뒤 파일에 없는 앱을 전부
숨겨서 화면에 선언한 것만 남긴다. 종료하지 않고 닫지도 않는다. Finder와 백그라운드 앱은
건드리지 않고, 숨긴 앱은 누르면 바로 돌아온다. 몇 개를 숨겼는지, 어떤 앱인지 보고에 적힌다.
...
전체화면 항목은 적힌 순서대로 하나씩 열려서, 새 Space가 기존 것들 뒤에 그 순서대로
줄을 선다. 이미 있는 Space의 순서는 건드리지 않고, 저장할 때도 전체화면 창을 Mission
Control 순서대로 담는다. 시스템 설정의 "최근 사용 내역에 따라 Space 자동 재정렬"이
켜져 있으면 macOS가 순서를 다시 섞으므로, 순서를 유지하려면 꺼둔다.

Split View, 즉 두 앱이 한 전체화면 Space를 나눠 쓰는 상태는 전체화면 항목 두 개로 저장된다.
config에 "이 둘이 한 Space를 나눈다"고 적을 방법이 없기 때문이다. 워크스페이스를 열면 각각
전용 전체화면이 된다. 저장할 때 그 사실을 알려준다.

**창이 여러 개인 앱** — `title`로 어느 창을 옮길지 지정한다. 제목의 일부면 된다. 없으면 아직
다른 항목이 잡지 않은 창 중 가장 최근 활성 창을 고르므로, 같은 앱을 두 번 적으면 서로 다른
창 두 개가 배치된다.
...
탭이 전부 빈 페이지인 창 — 브라우저가 켜지며 여는 시작 창 — 은 버리지 않고 재활용하므로,
브라우저를 새로 띄워도 빈 창이 남지 않는다.

**백업** — 워크스페이스 파일을 덮어쓰거나 단축키를 바꾸거나 삭제하기 직전에, 이전 내용이
`~/.config/restage/.backups/`에 복사되고 워크스페이스마다 최신 10벌이 유지된다. 패널에서
삭제하면 지금처럼 휴지통으로도 이동한다.
...
<details>
<summary><b>저장할 때 잡히는 것</b></summary>

- 브라우저는 **열려 있는 탭까지 함께 담는다.** 시작 페이지나 새 탭은 담지 않는다
- 자리가 애매하면 `?`가 붙는다. **조용히 추측하지 않는다**
- 같은 앱의 창 여러 개도 전부 담는다. 제목이 다른 것은 제목으로, 나머지는 순서대로 자리를
  잡으며 `Cursor 1`, `Cursor 2`로 보인다
- 시작 페이지뿐인 브라우저는 주소 없이 담긴다. 편집에서 주소 수를 눌러 넣는다
- 안 켜둔 앱은 `+`로, 지금 안 열려 있는 URL은 `w`로 추가한다

메뉴바 편집기에서는 창 이름을 누르면 그 창이 앞으로 와서 어느 것인지 알 수 있고, **웹**을
고르면 붙여 넣은 주소로 브라우저를 더한다.

</details>

<details>
<summary><b>메뉴바 자세히</b></summary>
...
**순환 단축키**는 키 하나로 다음 워크스페이스로 넘어간다. 순서는 목록에 보이는 그대로다.
마지막으로 연 것은 앱이 기억하고 config 파일에는 남기지 않는다. 그 카드에는 점이 붙는다.

**언어와 화면 모드**는 기본이 시스템을 따른다. 목록 아래에 **한국어 · English**가 있고,
톱니의 **화면 모드**는 시스템 설정 · 라이트 · 다크 중 고른다. 어느 쪽이든 바로 바뀐다.
패널을 띄워 둔 채 바꿔도 그 자리에서 바뀐다.

**업데이트**는 **업데이트 확인**을 누를 때만 GitHub에 물어본다. 주기적으로 확인하지 않는다.
이 도구를 쓰는 데 네트워크가 필요 없어야 하기 때문이다. 새 버전이 있으면 받는 방법을
알려준다. Homebrew로 설치했으면 그 명령을, 아니면 릴리스 페이지를 안내한다.
...
**아예 전체화면이 안 되는 창도 있다.** TextEdit의 문서 창이 그렇다. 접근성이 전체화면 속성을
설정 불가로 보고하고, 창 버튼을 하나도 내보이지 않으며, 자기 메뉴의 보기 > 전체 화면 시작도
비활성으로 둔다. 그런 창에 `fullscreen: true`를 주면 `constrained`로 보고하고 창은 그대로 둔다.

**다른 데스크탑의 창은 화면을 그쪽으로 보내 다룬다.** 창을 다른 데스크탑으로 옮기는 것은
공개 API로도 비공개 API로도 되지 않는다. 창 서버의 비공개 함수를 직접 불러 확인했고, 다른 앱의
창에는 조용히 무시된다. yabai 같은 도구가 SIP 부분 해제를 요구하는 이유다. 되는 것은 창이 있는
Space로 화면을 보내는 쪽이다. restage는 그렇게 보내 창을 놓고, 끝나면 원래 보던 곳으로
되돌린다. 시스템 설정은 필요 없다. 앱을 활성화하는 공개 방법들은 화면을 전혀 옮기지 못했고
하나씩 잰 기록이 [findings](docs/findings.md#spaces)에 있다.

화면을 보내는 데는 창 서버의 비공개 함수를 쓴다. macOS 업데이트로 막히면 창에 닿지 못한 사유만
남기고 창은 건드리지 않는다.
...
[findings.md](docs/findings.md)가 요약본이다. 실제 기계에서 재본 것들 중 코드를 읽어서는
알 수 없는 것만 모았다.
