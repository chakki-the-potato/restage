...
- **cursor 미검증**. Claude Code가 Cursor 확장으로 실행되므로 probe가 Cursor를 종료하면 세션이 함께 끊긴다. iTerm에서 별도로 확인해야 한다.

```
swift run restage probe --app cursor --slot left-half
```

- Task 8·9 진행 중 단일 창 배치로는 Cursor에서 이미 성공을 확인했다(첫 시도, 105ms, 외장 모니터에서 내장 화면 좌측 절반으로 이동). 다만 콜드/웜 2케이스 정식 검증은 아니다.
